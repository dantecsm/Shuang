...
function uploadDeploy() {
  ...
  try {
    ...
    // 使用scp上传文件
    const scpCmd = `scp -r ${config.deployDir}/* ${config.host}:${config.remotePath}/`;
    ...
    log('文件上传完成！');
    log(`访问地址: http://morefine${config.remotePath.replace('/opt/websites', '')}`);
...
命令:
  node scripts/deploy.js prepare  - 准备部署文件
  node scripts/deploy.js upload   - 上传文件到主机
  node scripts/deploy.js help     - 显示帮助信息

快捷命令:
  npm run deploy:prepare  - 准备部署文件
  npm run deploy:upload   - 上传文件到主机
...
switch (command) {
  ...
  case '-h':
    ...
    break;
  default:
    error('未知命令，使用 "node scripts/deploy.js help" 查看帮助');
}
